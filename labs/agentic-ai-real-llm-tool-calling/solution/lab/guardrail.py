...
def _iter_strings(value: Any):
    ...
    leaf. Tool arguments can nest, e.g. {"filters": {"query": "..."}}, so a
    flat ``.get("query")`` check would miss an injection buried one level
    deeper."""
...
def check_tool_arguments(tool_name: str, arguments: dict[str, Any]) -> GuardrailResult:
    ...
    injection pattern. ``tool_name`` is only used in the reason message here;
    a stricter production guardrail might vary sensitivity per tool (e.g. a
    destructive tool gets a lower bar), but a single strict policy for every
    tool is the safe default."""
