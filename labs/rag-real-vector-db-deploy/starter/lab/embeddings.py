...
class TfidfEmbedder:
    ...
    Vectors must be L2-normalised, so a plain dot product between two embeddings equals
    ...
    def fit(self, corpus: list[str]) -> None:
        ...
        TODO:
        - raise ValueError if corpus is empty (cannot fit a vectoriser on nothing)
        - fit_transform the vectorizer on corpus, record the resulting dimensionality
          in self.dim, and set self._fitted = True
        """
    ...
    def embed(self, texts: list[str]) -> np.ndarray:
        ...
        - return a (0, self.dim) array for an empty texts list (no transform needed)
        - otherwise: transform texts with the fitted vectorizer, convert to a dense
          float64 array, L2-normalise each row (guard against dividing by a zero norm)
        """
        raise NotImplementedError
