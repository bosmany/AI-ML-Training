...
def make_clients(cfg: StackConfig) -> dict[str, Any]:
    ...
    - Lambda function creation and cold starts can take longer than boto3's 60s default read
      timeout under load; give it more room.
    """
    common = dict(
    ...
    return {
        "s3": boto3.client("s3", config=Config(s3={"addressing_style": "path"}), **common),
        "iam": boto3.client("iam", **common),
        "lambda": boto3.client(
            "lambda", config=Config(connect_timeout=10, read_timeout=90), **common
        ),
    }
